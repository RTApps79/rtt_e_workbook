...
/* ============================ LEARNING STATIONS ===================== */
/* Each station is either an external activity (iframe) or a native module. */
const STATIONS = {
  lobby: [
    { name:'Patient Journey Sequencer', sub:'Order the radiation therapy workflow', icon:'🚶', type:'iframe', src:'patient_journey_sequencer_activity.html' }
  ],
  exam: [
    { name:'Vital Signs Trainer', sub:'Guided correct-order procedure', icon:'❤', type:'vitals' },
    { name:'Patient Care Skills Challenge', sub:'ID, body mechanics, PPE, AED', icon:'🩺', type:'iframe', src:'patient_care_skills_activity.html' },
    { name:'Patient Safety & ID Lab', sub:'Two-identifier verification scenarios', icon:'🛡', type:'iframe', src:'patient_safety_id_lab.html' }
  ],
  ct: [
    { name:'Patient Safety & ID Lab', sub:'Verify identity before simulation', icon:'🛡', type:'iframe', src:'patient_safety_id_lab.html' }
  ],
  phys: [
    { name:'Cell Division Microscope', sub:'Mitosis & radiobiology context', icon:'🔬', type:'iframe', src:'mitosis_microscopev2.html' }
  ],
  vault: [
    { name:'Patient Safety & ID Lab', sub:'Accessory & treatment-site verification', icon:'🛡', type:'iframe', src:'patient_safety_id_lab.html' }
  ]
};

function stationsHTML(id) {
  const list = STATIONS[id]; if (!list || !list.length) return '';
  let h = `<div class="grp"><div class="gt">Learning Stations</div>`;
  list.forEach((s, i) => {
...
function openStation(s) {
  ...
  if (s.type === 'vitals') {
    ...
    // iframe with graceful fallback if blocked (e.g., opened via file://)
    ...
    const fb = () => {
      bodyEl.innerHTML = `<div class="iframe-fallback">
        ...
        This activity lives in <code>${s.src}</code>. Make sure that file sits in the same folder as this page,
        and run it through a local server (browsers block embedded pages on <code>file://</code>):<br><br>
        <code>python -m http.server 8000</code> &nbsp;→&nbsp; open <code>http://localhost:8000/</code><br><br>
        You can also open <code>${s.src}</code> directly in its own tab.</div>`;
    };
    fr.addEventListener('error', fb);
    // some browsers silently blank the frame on file://; offer a manual link too
    setTimeout(() => { try { if (fr && !fr.contentWindow) fb(); } catch (e) { /* cross-origin ok */ } }, 1200);
  }
